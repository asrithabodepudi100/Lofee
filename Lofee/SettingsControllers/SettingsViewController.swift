...
//Variables
    let chooseSetToStudyTransparentView = UIView()
...
//IBActions
    ...
    @IBAction func openMenuButtonPressed(_ sender: UIButton) {
        print ("IM SO FUCKING TIRED OF THIS")
    ...
    @IBAction func startNotificationsToggleChanged(_ sender: UISwitch) {
        ...
        if (self.chooseSetToStudyLabel.text!) == "" ||  (self.chooseSetToStudyLabel.text!) == "Choose Set To Study "{ //if no set has been selected, but user tried to toggle notifications
            ...
            if self.startNotificationsToggle.isOn {
                ...
                //MUST TEST, but SHOULD
                    //...print ("false" when device is locked
                    //true when is unlocked
            }
            else {
                stopNotifications()
            }
...
extension SettingsViewController{
    func addTransparentView(frames: CGRect){
        ...
        chooseSetToStudyTransparentView.frame = self.view.frame
        self.view.addSubview(chooseSetToStudyTransparentView)
        tableView.frame = CGRect(x: frames.origin.x, y: frames.origin.y + frames.height, width: frames.width, height: 0)
        self.view.addSubview(tableView)
        tableView.layer.cornerRadius = 18
        chooseSetToStudyTransparentView.backgroundColor = UIColor.black

        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(removeTransparentView)).self
        chooseSetToStudyTransparentView.addGestureRecognizer(tapGesture)
        chooseSetToStudyTransparentView.alpha = 0
        UIView.animate(withDuration: 0.4, delay: 0.0, usingSpringWithDamping: 1.0, initialSpringVelocity: 1.0, options: .curveEaseInOut, animations: {
            self.chooseSetToStudyTransparentView.alpha = 0.5
    ...
    @objc func removeTransparentView(){
        ...
        UIView.animate(withDuration: 0.4, delay: 0.0, usingSpringWithDamping: 1.0, initialSpringVelocity: 1.0, options: .curveEaseInOut, animations: {
            self.chooseSetToStudyTransparentView.alpha = 0
